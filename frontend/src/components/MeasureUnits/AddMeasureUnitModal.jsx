...
function AddMeasureUnitModal(props) {
	...
	useEffect(() => {
		setDescriptionText(
			props.editMeasureUnit.length !== 0 ? 'editada' : 'agregada'
		);
		setMessageText(
			props.editMeasureUnit.length !== 0
				? 'Todos los cambios fueron registrados.'
				: ' La Unidad de Medida fue agregada exitosamente.'
		);
	}, [props]);
	return (
		<Dialog onClose={handleClose} open={open} fullScreen={fullScreen}>
			...
			<DialogTitle style={{ textAlign: 'center' }}>
				Unidad de Medida {descriptionText} exitosamente.
			</DialogTitle>
			<DialogContent>
				<DialogContentText
...
export default AddMeasureUnitModal;
