...
import AddMeasureUnitPreview from './AddMeasureUnitPreview';
import AddMeasureUnitModal from './AddMeasureUnitModal';
...
function AddMeasureUnit() {
	...
	const measureUnitData = useMemo(
		() => location.state?.measureUnitData || [],
		[location.state?.measureUnitData]
	);
	...
	const [nameState, dispatchName] = useReducer(nameReducer, {
		value: measureUnitData.name ? measureUnitData.name : '',
		isValid: true,
		feedbackText: '',
	});
	...
	const handleNext = async e => {
		...
		if (formIsValid && !isForm && measureUnitData.length === 0) {
			handleSubmit();
		}
		if (formIsValid && !isForm && measureUnitData.length !== 0) {
			handleEdit();
	...
	return (
		<>
			<Fragment>
				...
				{isForm ? (
					<div className={classes.listContainer}>
						...
						<FormControl fullWidth onSubmit={handleSubmit}>
							<Box mt={4}>
								<h6>1. Datos de la Unidad de Medida</h6>
				...
				{showModal && (
					<AddMeasureUnitModal editMeasureUnit={measureUnitData} />
				)}
			</Fragment>
		</>
	);
