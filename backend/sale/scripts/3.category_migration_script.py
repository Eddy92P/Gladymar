...
CATEGORY_URL = f"{os.getenv('BASE_URL')}/api/sale/categories/"
EXCEL_PATH = 'sale/import_files/categories.xlsx'
...
for index, row in df.iterrows():
    ...
    response = session.post(CATEGORY_URL, json=payload)
...
if failed:
    pd.DataFrame(failed).to_csv('failed_categories.csv', index=False)
    print("\n❌ Detalles de los registros fallidos han sido guardados en failed_categories.csv")
