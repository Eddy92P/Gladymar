...
MEASURE_UNIT_URL = f"{os.getenv('BASE_URL')}/api/sale/measure-units/"
EXCEL_PATH = 'sale/import_files/measure_unit.xlsx'
...
for index, row in df.iterrows():
    ...
    response = session.post(MEASURE_UNIT_URL, json=payload)
...
if failed:
    pd.DataFrame(failed).to_csv('failed_measure_units.csv', index=False)
    print("\n❌ Detalles de los registros fallidos han sido guardados en failed_measure_units.csv")
